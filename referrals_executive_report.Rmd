...
referralFact_raw <- referralFact_conn %>%
  mutate(StartInstant = as.Date(StartInstant)) %>%
  filter(StartInstant >= as.Date("2022-04-01") & StartInstant <= report_run_date) %>%
  select(ReferralKey, StartInstant, `_CreationInstant`, `_LastUpdatedInstant`, LastEnteredWorkqueueDateKey_X,
...
# status_validation <- referral_vol %>% group_by(Status, CloseReason, ApptStatusFinal) %>% summarise(total = n()) 
# write.xlsx(status_validation, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/Referral Status Validation.xlsx")
