...
prev_year <- as.numeric(format(as.Date(report_run_date), "%Y"))-1
...
# referrals_32bj <- referral_vol %>%
#   filter(IS32BJ_YN == "Y")
# 
# require(openxlsx)
# write.xlsx(referrals_32bj, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/32BJ Referral Data_1.12.2023.xlsx")
# 
# 
...
```{r Referrals Data Validation Output, echo = FALSE, warning = FALSE, message = FALSE}

# referrals_mss_validation <- referrals_conn %>%
#   mutate(StartInstant = as.Date(StartInstant)) %>%
#   filter(StartInstant >= as.Date("2022-07-01") & StartInstant <= as.Date("2022-07-31")) %>%
#   filter(Status != "Canceled") %>%
#   collect()
# 
# referrals_vol_mss_validation <- referral_vol %>%
#   filter(StartInstant >= as.Date("2022-07-01") & StartInstant <= as.Date("2022-07-31")) %>%
#   group_by(SentByDept, SentByDepID, Status) %>%
#   summarise(total_referrals_created = n())
# 
# 
# require(openxlsx)
# list_of_datasets <- list("Referrals Raw" = referrals_mss_validation, "Created Vol Jul22" = referrals_vol_mss_validation)
# write.xlsx(list_of_datasets, file = "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/MSHS Referrals Data Jul22.xlsx")

```


```{r Referrals Status and Reasons, echo = FALSE, warning = FALSE, message = FALSE}

# rfl_status_validation <- referral_vol %>%
#   group_by(Status, CloseReason, SchedulingStatus, ApptStatusFinal) %>%
#   summarise(total = n())
# 
# write.xlsx(rfl_status_validation, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/RFL Status Validation.xlsx")
...
# Unique Referral ID in Referrals Table 
# unique_ref <- referral_vol %>%
#   group_by(ReferralKey) %>%
#   summarise(inRefData = "Yes") 
# 
# wq_history_merged$inRefData <- unique_ref$inRefData[match(wq_history_merged$ReferralKey, unique_ref$ReferralKey)]
# 
...
wq_history_merged$releaseDate <- case_when(is.na(wq_history_merged$releaseDate) ~ Sys.Date() + 100,
                                           TRUE ~ wq_history_merged$releaseDate) 


wq_history_merged <- wq_history_merged %>%
  arrange(EventDateTime) %>%
  arrange(ReferralKey) %>%
  group_by(ReferralKey, EpicWorkqueueId, EventDateTime) %>%
  mutate(ID = seq_len(n())) %>%
...
```{r Referrals Snapshot Processing, echo = FALSE, warning = FALSE, message = FALSE}
...
referral_expired_ytd <- referral_vol %>%
  # filter(createdYear == reporting_year) %>%
  filter(referral_age >180 & ApptStatusFinal == "Pending") %>%
  # filter(ApptStatusFinal == "Pending" & as.Date(ExpirationInstant, "%Y-%m-%d") < report_run_date) %>%
  group_by(createdYear) %>%
  summarise(total = n(),
            total_prof_charge_est = sum(ProfessionalChargeEstimate, na.rm = T),
...
referral_outcome_ytd <- referral_vol %>%
  # filter(createdYear == reporting_year) %>%
  filter(Class != "Outgoing") %>%
  group_by(createdYear, ApptStatusFinal) %>%
  summarise(total = n()) %>%
...
*-- ReferralFact*<br/>
*-- VisitFact*<br/>
*-- ReferralWorkqueueEventFactX*<br/>
*-- ReferralWQReason*<br/>
_________________________________________________________________________________________________________________________________________________________________________________________________
<br/>

## 1. Referrals Snapshot
### `r reporting_year` {.tabset .tabset-fade .tabset-pills}
<br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2023-01-01 to `r report_run_date`*</span><br/>
```{r Current Year Referrals Snapshot Output 1, echo = FALSE, warning = FALSE, message = FALSE, fig.width=12}
...
referrals_snapshot_1 <- data.frame(
  total = prettyNum((referral_received_ytd %>% filter(createdYear == reporting_year))$total, big.mark = ","), # Total Referrals Created
  blank_1 = "",
  rate = paste0(paste0((referral_outcome_ytd %>% filter(createdYear == reporting_year))$conversion_rate,"%")," | ", paste0((referral_outcome_ytd %>% filter(createdYear == reporting_year))$completion_rate,"%")), # Conversion | Completion Rate 
  blank_2 = "",
  contacted = paste0(round((referral_contacted_ytd %>% filter(createdYear == reporting_year))$perc,2)*100,"%"), # Contacted/ Scheduled <12 Hours
  blank_3 = "",
  onTime = paste0(round((referral_timeframe_ytd %>% filter(createdYear == reporting_year))$perc,2)*100,"%") # Referrals Scheduled on-time based on priority
)


reactable(
    referrals_snapshot_1,
    style = list(fontFamily = 'Calibri', fontSize = '20px'),
    ...
    rowStyle = function(index) {
            if (index %in% c(1)) {
              list(`border-top` = "2px solid rgb(255,255,255)",
                   fontWeight = "Bold")
            }
          },
    ...
    columns = list(
      total = colDef(
        ...
        style = list(fontWeight = "Bold", color ="black", background = "#ebebf9"),
        align = "center"
      ...
      blank_3 = colDef(
       ...
       onTime = colDef(
        name = "% Referrals Scheduled On-Time",
        maxWidth = 300,
        headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "20px"),
        style = list(fontWeight = "Bold", color ="black", background = "#ebebf9"),
        align = "center"
...
```{r Current Year Referrals Snapshot Output 2, echo = FALSE, warning = FALSE, message = FALSE, fig.width=12}

# Referrals YTD Snapshot Tables ------------------------------------------------
referrals_snapshot_2 <- data.frame(
  class = paste0(paste0(round((referral_vol_class_ytd %>% filter(createdYear == reporting_year))$Incoming*100,0),"%")," | ", paste0(round((referral_vol_class_ytd %>% filter(createdYear == reporting_year))$Outgoing*100,0),"%")),
  blank_1 = "",
  
  blank_2 = "",
  onTime = paste0(round(referral_timeframe_ytd$perc,2)*100,"%"),
  blank_3 = "",
  expired = paste0(prettyNum((referral_expired_ytd %>% filter(createdYear == reporting_year))$total, big.mark = ",")," | ", 
                   paste0(round((referral_expired_ytd %>% filter(createdYear == reporting_year))$total / referral_outcome_ytd$Total,2)*100,"%"))
  blank_1 = "",
  expired_perc = paste0(round(referral_expired_ytd$total / referral_outcome_ytd$Total,2)*100,"%"),
  
  
   class = paste0(paste0(round(referral_vol_class_ytd$Incoming,3)*100,"%")," | ", paste0(round(referral_vol_class_ytd$Outgoing,3)*100,"%")), # Incoming | Outgoing Referrals 
  blank_3 = "",
  
  charges = paste0("$",comprss(referral_est_rev_ytd$total_prof_charge_est + referral_est_rev_ytd$total_hosp_charge_est)), # Total estimated charges generated 
)
    ...
    columns = list(
      class = colDef(
        name = "% Incoming | Outgoing Referrals",
         maxWidth = 300,
        headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "20px"),
        style = list(fontWeight = "Bold", background = "#e6f8ff", color = "black"),
        align = "center"
        # footer = "*Excludes MSDD and NETWORK"
        ),
      blank_1 = colDef(
...
```{r Referrals YTD Snapshot Output 3, echo = FALSE, warning = FALSE, message = FALSE, fig.width=12}
...
reactable(
    ...
    columns = list(
      expired_cnt = colDef(
        ...
        maxWidth = 300,
        headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "22px"),
        style = list(fontWeight = "Bold", background = "#fee7f5", color = "black"),
        align = "center"
        # footer = "*Excludes MSDD and NETWORK"
        ),
      blank_1 = colDef(
...
hchart(referral_received_monthly_mshs, 
  ...
  hc_title(text = "Referral Conversion and Completion Rate", align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  ...
  hc_caption(text = "<em>*Conversion Rate: Total referrals with visits scheduled / Total referrals created<em/>;
             Completion Rate: Total referrals with arrived visits / Total referrals created<em/>")
...
### Snapshot
<br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Workqueues as of `r report_run_date`*</span><br/>
```{r WQ Snapshot Output 1, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
    ...
    columns = list(
      total = colDef(
        ...
        minWidth = 300,
        headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "22px"),
...
```{r WQ Snapshot Output 2, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
    ...
    columns = list(
      urgent = colDef(
        ...
        minWidth = 300,
        headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "22px"),
        style = list(fontWeight = "Bold", background = "#00aeef", color = "white"),
        align = "center"
        # footer = "*Excludes MSDD and NETWORK"
        ),
      blank_1 = colDef(
...
```{r WQ Snapshot Output 3, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
    ...
    columns = list(
      wq_added = colDef(
        name = paste0("Avg Referrals Added per Weekday (Week of ", reporting_week,")"),
        minWidth = 300,
        headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "22px"),
      ...
      wq_removed = colDef(
        name = paste0("Avg Referrals Removed per Weekday (Week of ",reporting_week,")"),
        minWidth = 300,
        headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "22px"),
        style = list(fontWeight = "Bold", background = "#d80b8c", color = "white"),
        align = "center"
      ...
      wq_added_wknd = colDef(
        name = paste0("Total Referrals Added during Weekend (Week of ",reporting_week,")"),
        minWidth = 300,
        headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "22px"),
        style = list(fontWeight = "Bold", background = "#d80b8c", color = "white"),
        align = "center"
...
hchart(referral_per_patient, 
  ...
  hc_add_series(name = "Avg ED Referrals per Patient",
  ...
  hc_tooltip(shared = TRUE, borderColor = "black") %>%
  hc_caption(
    text = "<em>*Average referrals per patient calculated based on Total referrals created / Total unique patients referred</em>"
  )
...
### Scheduling Priority
<span style='font-size: 22px; font-weight: bold; color: black'>*ED Referrals by Scheduling Priority*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date`*</span><br/>
```{r ED Referrals Total Volume by Priority, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

ed_vol_priority_cnt_rows <- referral_vol %>%
...
ed_vol_priority_cnt <- as.data.frame(ed_vol_priority_cnt_rows)
...
reactable(
  ...
  style = list(fontFamily = 'Calibri', fontSize = 'px14'),
    ...
    bordered = TRUE,
  rowStyle = function(index) {
            if (index %in% nrow(ed_vol_priority_cnt)) {
              list(`border-top` = "2px solid rgb(184,184,184)",
                   fontWeight = "Bold",
                   background = "white")
            }
    ...
    style = function(value, index) {
      if (ed_vol_priority_cnt_rows$priority_type[index] == "Total") return()
      normalized <- (value - min(ed_vol_priority_cnt[1:(nrow(ed_vol_priority_cnt)-1),])) / (max(ed_vol_priority_cnt[1:(nrow(ed_vol_priority_cnt)-1),]) - min(ed_vol_priority_cnt[1:(nrow(ed_vol_priority_cnt)-1),]))
    ...
    align = "center",
    minWidth = 50,
    headerStyle = list(fontWeight = "Bold", fontSize = "16px", color = "white", background = "#212070")
  ),
    columns = list(
...
<span style='font-size: 22px; font-weight: bold; color: black'>*% of ED Referrals by Scheduling Priority*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date`*</span><br/>
```{r Percent of ED Referrals by Scheduling Priority, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

ed_vol_priority_perc_rows <- referral_vol %>%
  filter(SentBySite %in% c("ED","PSYCH ED")) %>%
  group_by(createdMonthYear, priority_type) %>%
  summarise(total = n()) %>%
...
ed_vol_priority_perc <- as.data.frame(ed_vol_priority_perc_rows)
...
### Conversion Rate 
<br/>
```{r ED VS. Non ED Referrals Conversion Rate by Priority, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

ed_conversion_month <- referral_vol %>%
  mutate(ed = ifelse(SentBySite %in% c("ED","PSYCH ED"), "ED","Non-ED")) %>%
  group_by(createdMonthYear, ed, priority_type, ApptStatusFinal) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = ApptStatusFinal,
              values_from = total,
              values_fill = 0) %>%
...
ed_conversion <- referral_vol %>%
  mutate(ed = ifelse(SentBySite %in% c("ED","PSYCH ED"), "ED","Non-ED")) %>%
  group_by(ed, priority_type, ApptStatusFinal) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = ApptStatusFinal,
              values_from = total,
              values_fill = 0) %>%
...
ed_conversion_rate_graph <- 
  hchart(ed_conversion, 
       hcaes(x = 'priority_type', y = 'conversion_rate', group = 'ed'), 
       type = 'column',
       color=c("#a5a7a5", "#212070"),
       dataLabels = list(enabled = TRUE, format='{point.y}%'), 
       showInLegend = TRUE) %>%
  hc_yAxis(
    title = list(text = "Converesion Rate"),
    opposite = FALSE
  ...
  hc_xAxis(title = "Referral Scheduling Priority") %>%
  hc_legend(align = "left", verticalAlign = "top", itemStyle = list(fontFamily ='Calibri',color ='black', fontSize = "14px")) %>%
  hc_title(text = "ED vs. Non-ED Referrals Conversion Rate", align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  hc_subtitle(text = paste0("Based on referrals created from ","2022-04-01"," to ",report_run_date),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black") %>%
  hc_caption(text = "<em>*Conversion Rate: Total referrals with visits scheduled / Total referrals created<em/>")
...
ed_completion_rate_graph <- 
  hchart(ed_conversion, 
       hcaes(x = 'priority_type', y = 'completion_rate', group = 'ed'), 
       type = 'column',
       color=c("#d80b8c", "#00aeef"),
       dataLabels = list(enabled = TRUE, format='{point.y}%'), 
       showInLegend = TRUE) %>%
  hc_yAxis(
  ...
  hc_xAxis(title = "Referral Scheduling Priority") %>%
  hc_legend(align = "left", verticalAlign = "top", itemStyle = list(fontFamily ='Calibri',color ='black', fontSize = "14px")) %>%
  hc_title(text = "ED vs. Non-ED Referrals Completion Rate", align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  hc_subtitle(text = paste0("Based on referrals created from ","2022-04-01"," to ",report_run_date),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black") %>%
  hc_caption(text = "<em>*Completion Rate: Total referrals with arrived visits / Total referrals created<em/>")

hw_grid(ed_conversion_rate_graph, ed_completion_rate_graph)

```


<span style='font-size: 22px; font-weight: bold; color: black'>*ED vs. Non-ED Conversion Rate by Month*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date``*</span><br/>
```{r ED vs. Non ED Converion Rate Table, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

ed_conversion_rate <- ed_conversion_month %>%
  dplyr::select(createdMonthYear, ed, priority_type, conversion_rate) %>%
  mutate(conversion_rate = conversion_rate/100) %>%
  pivot_wider(names_from = createdMonthYear,
              values_from = conversion_rate) 

ed_conversion_rate <- as.data.frame(ed_conversion_rate)
rownames(ed_conversion_rate) <- paste0(ed_conversion_rate$priority_type, ": ",ed_conversion_rate$ed)
ed_conversion_rate  <- ed_conversion_rate[,3:length(ed_conversion_rate)]
...
reactable(
  ...
  defaultColDef = colDef(
    style = function(value, index) {
      ...
      normalized <- (value - min(ed_conversion_rate)) / (max(ed_conversion_rate) - min(ed_conversion_rate))
      color <- BuYlRd(normalized)
      list(background = color, fontWeight = "Bold")
    },
    format = colFormat(digits = 0, percent = TRUE),
...
<span style='font-size: 22px; font-weight: bold; color: black'>*ED vs. Non-ED Completion Rate by Month*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date``*</span><br/>
```{r ED vs. Non ED Completion Rate Table, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

ed_completion_rate <- ed_conversion_month %>%
  dplyr::select(createdMonthYear, ed, priority_type, completion_rate) %>%
  mutate(completion_rate = completion_rate/100) %>%
  pivot_wider(names_from = createdMonthYear,
              values_from = completion_rate) 

ed_completion_rate <- as.data.frame(ed_completion_rate)
rownames(ed_completion_rate) <- paste0(ed_completion_rate$priority_type, ": ",ed_completion_rate$ed)
ed_completion_rate  <- ed_completion_rate[,3:length(ed_completion_rate)]
...
reactable(
  ...
  defaultColDef = colDef(
    style = function(value, index) {
      if (is.na(value)) return()
      normalized <- (value - min(ed_completion_rate)) / (max(ed_completion_rate) - min(ed_completion_rate))
      color <- BuYlRd(normalized)
      list(background = color, fontWeight = "Bold")
    },
    format = colFormat(digits = 0, percent = TRUE),
...
### Sent To Specialty
<span style='font-size: 22px; font-weight: bold; color: black'>*ED Referrals by Original vs. Final Sent To Specialty*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date`*</span><br/>
```{r ED Referrals Original vs. Final Sent To Specialty, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

# ed_referrals_sent_to <- referral_vol %>%
#   filter(SentBySite %in% c("ED","PSYCH ED"))
...
<span style='font-size: 22px; font-weight: bold; color: black'>*ED Referrals by Primary Diagnosis and Sent To Specialty*</span><br/>
<span style='font-size: 18px; font-weight: bold; color: black'>*Based on Referrals Created from 2022-04-01 to `r report_run_date`*</span><br/>
```{r ED Referrals Primary Diagnosis by Receiving Specialty, echo = FALSE, warning = FALSE, message = FALSE, fig.width=13, fig.align='center'}

top20_ed_diagnosis <- referral_vol %>%
  filter(SentBySite %in% c("ED","PSYCH ED")) %>%
  group_by(ICD_10_Chosen) %>%
  summarise(total = n()) %>%
  arrange(desc(total)) %>%
  head(20)

ed_referral_vol_diagnosis_specialty <- referral_vol %>%
  filter(ICD_10_Chosen %in% as.vector(top20_ed_diagnosis$ICD_10_Chosen)) %>%
  group_by(ICD_10_Chosen, ReceivedBySpecialty) %>%
  summarise(total = n()) %>%
  group_by(ICD_10_Chosen) %>%
  mutate(perc = total/sum(total)) %>%
  arrange(desc(total))
...
reactable(
  ed_referral_vol_diagnosis_specialty,
  style = list(fontFamily = 'Calibri', fontSize = 'px14'),
  defaultColDef = colDef(
    format = colFormat(digits = 0, percent = TRUE),
    minWidth = 50,
    ...
    wrap = TRUE,
    bordered = TRUE,
  groupBy = "ICD_10_Chosen",
  columns = list(
    ICD_10_Chosen = colDef(name = "Top 20 ICD 20 Codes Chosen for Referral",
                           style = list(fontWeight = "Bold"),
                           minWidth = 200),
    ReceivedBySpecialty = colDef(name = "Sent To Specialty",
                                 minWidth = 150),
    total = colDef(name = "Total Referrals Sent", aggregate = "sum", 
                   format = colFormat(separators = TRUE),
                   align = "center"),
    perc = colDef(name = "% of Total Referrals Sent with ICD 10",
                  format = colFormat(percent = TRUE, digits = 1),
                  align = "center")
    )
  )
