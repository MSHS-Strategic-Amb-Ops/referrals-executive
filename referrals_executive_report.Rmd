...
amb_mapping <- amb_mapping %>% collect()
write.xlsx(amb_mapping, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/amb_mapping.xlsx")
...
status_validation <- referral_vol %>% group_by(Status, SchedulingStatus, CloseReason, ApptStatusFinal) %>% summarise(total = n())
write.xlsx(status_validation, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/Referral Status Validation Updated.xlsx")

# write.xlsx(wq_reasons_validation, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/wq_reasons_validation.xlsx")
...
wq_history_merged <- left_join(referral_vol[,c("ReferralKey", "ReferralEpicId", "InAnyWorkqueue", "priority_type")], 
                               wq_history_conn %>% 
...
# Ambulatory Referrals Exeuctive Summary 
...
#### Referrals
```{r Referrals YTD Snapshot Output, echo = FALSE, warning = FALSE, message = FALSE,  results='hide', fig.width=12}
...
reactable(
    ...
    style = list(fontFamily = 'Calibri', fontSize = '26px'),
...
reactable(
    ...
    columns = list(
      ...
      expired_charges = colDef(
        ...
        minWidth = 300,
        headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "20px"),
        style = list(fontWeight = "Bold", color = "red"),
        align = "center"
        # footer = "*Excludes MSDD and NETWORK"
        )
    ) # Close Columns
...
```{r Monthly Volume Trending, echo = FALSE, warning = FALSE, message = FALSE, fig.width=8}
...
#### Workqueues
```{r WQ Snapshot Output, echo = FALSE, warning = FALSE, message = FALSE, results='hide'}
...
### 2. ED Referrals
```{r ED Referrals Analysis, echo = FALSE, warning = FALSE, message = FALSE}
